        ...
        <table class="table">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <div class="actions">
                  <a href="<?=APP_URL?>/admin/modules/orders/view.php?id=<?=$ord['id']?>" class="btn btn-sm btn-primary" data-tooltip="Detail"><i class="fas fa-eye"></i></a>
                  <button onclick="confirmDelete('<?=APP_URL?>/admin/modules/orders/delete.php?id=<?=$ord['id']?>&token=<?=csrfToken()?>','pesanan <?=htmlspecialchars(addslashes($ord['order_number']))?>')" class="btn btn-sm btn-danger" data-tooltip="Hapus"><i class="fas fa-trash"></i></button>
                </div>
              </td>
            </tr>
            <?php endforeach;?>
